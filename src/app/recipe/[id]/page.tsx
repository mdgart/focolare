    ...
    <article className="space-y-8 max-w-3xl">
      ...
      <div className="border-b border-neutral-800 pb-8">
        <div className="flex items-center gap-2 mb-3">
          <Link href={`/c/${ch.slug}`} className="text-sm font-medium text-amber-300 hover:text-amber-200 transition">
            {ch.title}
          </Link>
          {r.visibility === "members" && (
            <span className="text-xs px-2 py-1 bg-amber-900/40 text-amber-200 rounded-full font-medium">
              Member Only
            </span>
          )}
        </div>
          ...
          {(r.ingredients ?? []).length === 0 ? (
            ...
            <ul className="divide-y divide-neutral-700">
              {(r.ingredients ?? []).map((ing, i) => (
                <li key={i} className="p-4 hover:bg-neutral-800/50 transition">
                  <div className="flex items-center gap-3">
                    <input
                      type="checkbox"
                      className="h-5 w-5 rounded cursor-pointer accent-amber-500"
        ...
        <div className="space-y-4">
          {steps.length === 0 ? (
            ...
            steps.map((s, idx) => {
              ...
              return (
                <div key={s.id} className="card p-6 border-l-4 border-l-amber-500">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <span className="inline-flex items-center justify-center h-8 w-8 rounded-full bg-amber-500/20 text-amber-300 font-bold text-sm">
                          {idx + 1}
                        </span>
                        <h3 className="text-lg font-semibold text-neutral-100">{s.title}</h3>
                      </div>
                      <p className="text-neutral-300 mt-3 leading-relaxed">{s.instruction}</p>
                      {stepDuration && (
                        <div className="mt-4 inline-flex items-center gap-2 bg-orange-500/10 border border-orange-500/30 rounded-lg px-3 py-2">
      ...
      {/* Cook Mode CTA */}
      <div className="sticky bottom-4 sm:relative pt-8 border-t border-neutral-800">
        {session?.user ? (
          <StartCookForm recipeId={r.id} />
        ) : (
          <div className="bg-gradient-to-r from-neutral-800/50 to-neutral-900/50 border border-neutral-700 rounded-lg p-6 text-center">
