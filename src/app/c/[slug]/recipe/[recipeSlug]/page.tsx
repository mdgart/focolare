...
export default async function RecipePage({
  ...
  const id = await getRecipeIdBySlug(slug, recipeSlug);
  if (!id) notFound();
  ...
  // Built from the row rather than the params so a request that arrived with
  // odd casing still links to the canonical address.
  const selfPath = recipePath(ch.slug, r.slug);
  ...
  const recipePageUrl = host ? `${proto}://${host}${selfPath}` : "";
            ...
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <SaveRecipeButton
                  ...
                  signInHref={`/sign-in?next=${selfPath}`}
                />
                ...
                {canDeleteRecipe ? (
                  <>
                    ...
                    <Link
                      href={recipeEditPath(ch.slug, r.slug)}
          ...
          <div className="lg:sticky lg:top-24">
            {session?.user ? (
              ...
              <div className="rounded-3xl border border-sand bg-surface p-6 text-center">
                ...
                <Link href={`/sign-in?next=${selfPath}`} className="btn btn-primary w-full">
                  Sign in to cook
                </Link>
              </div>
            )}
